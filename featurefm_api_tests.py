...
Python implementation for VSCode
Complete API testing framework for Feature.fm music marketing platform
Updated with modern type hints (Python 3.10+)
"""
...
class FeatureFMAPITester:
    ...
    def __init__(self, api_key: str, secret_key: str, iss: str, verbose: bool = True):
        """
    ...
    def _generate_jwt(self, payload: Dict[str, Any] | None = None) -> str:
        ...
        payload.update({
            ...
            'jti': hashlib.md5(f"{time.time()}".encode()).hexdigest()
        })
    ...
    def test_jwt_auth(self) -> bool:
        ...
        if success:
            ...
            self._print_status(f"JWT authentication not required or failed", 
    ...
    def test_get_artist_details(self) -> bool:
        ...
        if success:
            ...
            self._print_status(f"Failed to get artist details", TestStatus.FAILED, 1)
    ...
    def test_create_smartlink(self) -> bool:
        ...
        if success:
            ...
            if isinstance(smartlink, dict):
                ...
                self._print_status(f"Smart link created successfully", TestStatus.PASSED, 1)
            ...
            self._print_status(f"Failed to create smart link", TestStatus.FAILED, 1)
    ...
    def test_list_smartlinks(self) -> bool:
        ...
        if success:
            ...
            self._print_status(f"Failed to list smart links", TestStatus.FAILED, 1)
    ...
    def test_get_smartlink_analytics(self) -> bool:
        ...
        if success:
            ...
            self._print_status(f"Analytics not available (may be normal for new links)", 
                             TestStatus.WARNING, 1)
    ...
    def test_create_presave_campaign(self) -> bool:
        ...
        if success:
            ...
            if isinstance(campaign, dict):
                ...
                self._print_status(f"Pre-save campaign created", TestStatus.PASSED, 1)
            ...
            if status_code == 404:
                ...
                self._print_status(f"Failed to create pre-save campaign", TestStatus.FAILED, 1)
    ...
    def test_list_campaigns(self) -> bool:
        ...
        if success:
            ...
            self._print_status(f"Failed to list campaigns or not available", TestStatus.WARNING, 1)
    ...
    def test_create_action_page(self) -> bool:
        ...
        if success:
            ...
            if isinstance(page, dict):
                ...
                self._print_status(f"Action page created", TestStatus.PASSED, 1)
            ...
            if status_code in [403, 404]:
                ...
                self._print_status(f"Failed to create action page", TestStatus.FAILED, 1)
    ...
    def test_create_release(self) -> bool:
        ...
        if success:
            ...
            self._print_status(f"Failed to create release", TestStatus.WARNING, 1)
    ...
    def test_create_webhook(self) -> bool:
        ...
        if success:
            ...
            if isinstance(webhook, dict):
                ...
                self._print_status(f"Webhook created", TestStatus.PASSED, 1)
                self._print_status(f"ID: {webhook_id}", indent=2)
                self._print_status(f"Events: {', '.join(webhook_data['events'][:2])}...", indent=2)
                self.test_data['webhook_id'] = webhook_id
            self._record_test('create_webhook', TestStatus.PASSED, response)
        else:
            self._print_status(f"Webhooks not available or require special access", 
                             TestStatus.WARNING, 1)
    ...
    def test_get_overview_analytics(self) -> bool:
        ...
        if success:
            ...
            self._print_status(f"Overview analytics not available", TestStatus.WARNING, 1)
